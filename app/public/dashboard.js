let apiKey = "";
      ...
      expenses.forEach(function (exp) {
        ...
        btn.addEventListener("click", async function () {
          const res = await fetch("/delete-expense", {
            ...
            body: JSON.stringify({ email: email, name: exp.name })
          });
          if (res.ok) {
            currentExpenses = currentExpenses.filter(function (e) { return e.name !== exp.name; });
    ...
    updateIncomeBtn.addEventListener("click", async function () {
      ...
      const res = await fetch("/update-income", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: email, income: income, budgetIndex: 0 })
      });
    ...
    addExpenseBtn.addEventListener("click", async function () {
      ...
      const res = await fetch("/add-expense", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: email, expenseName: name, expense: amount, budgetIndex: 0 })
      });
    ...
    addStockBtn.addEventListener("click", async function () {
      ...
      try {
        ...
        const res2 = await fetch("/add-stock", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email: email, symbol: symbol, amount: amount, budgetIndex: 0 })
        });
