...
// Client-side dashboard script (moved out of server.js).
// Keep this file in public/js/dashboard.js and include it from your dashboard HTML:
...
async function updateInvestmentAmount() {
  ...
  // compute stockValue concurrently via /quote
  if (currentStocks.length) {
    // map to concurrent fetch promises
    const promises = currentStocks.map(st => {
      const url = `/quote?symbol=${encodeURIComponent(st.symbol)}`;
      return fetchWithTimeout(url, { credentials: 'include' }, 4000)
        .then(r => r && r.ok ? r.json() : null)
        .catch(() => null)
        .then(data => ({ symbol: st.symbol, amount: Number(st.amount || 0), price: data?.currentPrice ?? 0 }));
    });

    const settled = await Promise.all(promises);
    for (const r of settled) {
      if (r && r.price) stockValue += Number(r.price) * Number(r.amount || 0);
    }
  }
...
async function renderStocks(stocks) {
  ...
  // Build concurrent quote fetches
  const promises = stocks.map(async (st) => {
    if (!st || !st.symbol || !st.amount) return null;
    const url = `/quote?symbol=${encodeURIComponent(st.symbol)}`;
    try {
      const res = await fetchWithTimeout(url, { credentials: 'include' }, 4000);
      if (!res || !res.ok) return { st, price: 0 };
      const data = await res.json();
      return { st, price: Number(data.currentPrice || 0) };
    } catch (e) {
      console.warn('renderStocks fetch error', st.symbol, e);
      return { st, price: 0 };
    }
  });
  ...
  for (const item of results) {
    ...
    const tr = document.createElement("tr");
    tr.innerHTML =
      `<td><a href="stockNews.html?stockSymbol=${st.symbol}">${st.symbol}</a></td>` +
      `<td>${st.amount}</td>` +
    ...
    btn.addEventListener("click", async () => {
      const resDel = await fetch("/delete-stock", {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ symbol: st.symbol }) // email removed
      });
...
async function loadStockPerformance(stocks) {
  ...
  const promises = stocks.map(st => fetchWithTimeout(`/quote?symbol=${encodeURIComponent(st.symbol)}`, { credentials: 'include' }, 4000)
    .then(r => r && r.ok ? r.json() : null)
    .catch(() => null)
    .then(data => ({ symbol: st.symbol, value: Math.floor((data?.currentPrice || 0) * Number(st.amount || 0)) }))
  );
...
async function loadStockHistory(stocks) {
  ...
  for (const st of stocks) {
    try {
      ...
      console.warn("loadStockHistory error for", st.symbol, e);
    }
  }
...
async function fetchStocks() {
  ...
  const promises = symbols.map(async (sym) => {
    try {
      ...
      const res = await fetchWithTimeout(`/whatIf?stockSymbol=${encodeURIComponent(yahooSym)}&period1=${getDateNMonthAgo(12)}&period2=${getDateNMonthAgo(0)}&interval=1d&shares=1`, { credentials: 'include' }, 5000);
      
      if (!res || !res.ok) {
        console.warn(`Failed to fetch stock ${sym}: ${res ? res.statusText : 'no response'}`);
        return null;
      }
...
async function runWhatIf(symbol, months, shares) {
  ...
  try {
    const res = await fetch("/whatIf?" + paramsW.toString(), { credentials: 'include' });
    if (res.ok) {
      const data = await res.json();
  ...
  // fallback to stock-history endpoint if whatIf fails
  ...
  // fetch current price via quote endpoint (yahoo)
  const qRes = await fetch(`/quote?symbol=${encodeURIComponent(symbol)}`);
  const qData = await qRes.json();
  const currentPrice = Number(qData.currentPrice || 0);
...
async function loadDashboard() {
  ...
  if (!res.ok) {
    ...
    console.error("Failed to load /dashboard:", res.status, text);
    document.body.innerHTML = "<h2>Error loading dashboard. Please log in.</h2>";
    return;
  }
  const data = await res.json();
  apiKey = data.apiKey || "";
...
addStockBtn.addEventListener("click", async function () {
  ...
  try {
    const url = `/quote?symbol=${encodeURIComponent(symbol)}`;
    const res = await fetchWithTimeout(url, { credentials: 'include' }, 4000);
    if (!res || !res.ok) { alert("Invalid stock symbol"); return; }
    const data = await res.json();
    if (!data || typeof data.currentPrice !== "number") { alert("Invalid stock symbol"); return; }
